...
void InitializePrecalc(StaticPokemonData& p)
{
	...
	// 倒すスコア
	float defeatScore = static_cast<float>(p.exp) / p.maxHp;
	// 捕まえるスコア
	float catchScore = static_cast<float>(p.totalDamage) / (p.maxHp - p.captureHp);
	// 最終スコア
...
int main()
{
	...
	for (int i = 1; i <= N; i++)
	{
		...
		cin >> p.maxHp >> p.captureHp >> p.exp
			>> p.movePower[0] >> p.maxMoveCount[0]
			>> p.movePower[1] >> p.maxMoveCount[1];
	...
	cin >> p0.movePower[0] >> p0.maxMoveCount[0]
		>> p0.movePower[1] >> p0.maxMoveCount[1];
	...
	while (true)
	{
		...
		if ((int)handIds.size() < 6)
		{
			...
			for (int ri = (int)sortedRank.size() - 1; ri >= 0; --ri)
			{
				...
				// 捕まえるために必要なダメージを今の手持ちで賄えるか確認
				// 賄えない場合はスキップして次の候補（もう少し先頭寄り）へ
				int requiredDmg = currentHp - captureHp;
